...
Describe 'Invoking PSScriptAnalyzer against commandbase' {
	...
	foreach ($file in $commandFiles) {
		Context "Analyzing $($file.BaseName)" {
			[array]$excludes = 'PSAvoidUsingInvokeExpression', 'PSAvoidTrailingWhitespace', 'PSShouldProcess', 'PSAvoidGlobalVars'

			$analysis = Invoke-ScriptAnalyzer -Path $file.FullName -ExcludeRule $excludes

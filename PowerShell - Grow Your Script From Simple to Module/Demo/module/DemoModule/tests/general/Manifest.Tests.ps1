Describe "Validating the module manifest" {
	...
	$manifest = ((Get-Content "$moduleRoot\DemoModule1.psd1") -join "`n") | Invoke-Expression
	[version]$moduleVersion = Get-Item "$moduleRoot\DemoModule1.psm1" | Select-String -Pattern '\$script:PSModuleVersion = "(.*?)"' | ForEach-Object { $_.Matches[0].Groups[1].Value }
